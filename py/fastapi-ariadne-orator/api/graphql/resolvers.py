...
@query.field('ninja')
async def resolve_ninja(obj, info, id):
    ninja = info.context['pg_dal'].get_ninja_with_related_jutsu(id)
    return ninja


@query.field('jutsu')
async def resolve_jutsu(obj, info, id):
...
def resolve_remove_known_jutsu(obj, info, ninja_id, jutsu_id):
    ninja_with_known_jutsu = (
        info.context['pg_dal']
