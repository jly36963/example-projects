...
const start = async (app) => {
    ...
    await (async () => {
        for (const route of routers) {
            ...
            const { default: router } = await Promise.resolve().then(() => __importStar(require(`./routes/${route}`)));
            app.use(router.routes()).use(router.allowedMethods());
        }
    })();
    app.use(async (ctx) => await koa_send_1.default(ctx, "./client/build/index.html"));
