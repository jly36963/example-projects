...
pub fn start() -> () {
    ...
    // Middleware
    let (chain, pipelines) = single_pipeline(new_pipeline().add(SimpleLogger::new(Level::Info)).add(middleware::ProvidersMiddleware).build());

    // Router
    let router = build_router(chain, pipelines, |r| {
        r.scope("/api/ninja", |r| {
            r.get("/:id/").with_path_extractor::<ninja::NinjaIdPathParams>().to_async(ninja::get_ninja);
            r.post("/").to_async(ninja::insert_ninja);
            r.put("/:id/").with_path_extractor::<ninja::NinjaIdPathParams>().to_async(ninja::update_ninja);
            r.delete("/:id/")
        ...
        r.scope("/api/jutsu", |r| {
            r.get("/:id/").with_path_extractor::<jutsu::JutsuIdPathParams>().to_async(jutsu::get_jutsu);
            r.post("/").to_async(jutsu::insert_jutsu);
            r.put("/:id/").with_path_extractor::<jutsu::JutsuIdPathParams>().to_async(jutsu::update_jutsu);
            r.delete("/:id/")
