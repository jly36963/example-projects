...
pub async fn get_ninja(
    ...
    Path(id): Path<String>,
) -> Result<Json<Ninja>, (StatusCode, String)> {
        ...
        Ok(n) => match n {
            ...
            None => {
                return Err((
                    ...
                    format!("Could not find ninja with id '{}'", id),
                ))
            }
        },
        Err(_) => {
...
pub async fn update_ninja(
    State(providers): State<Providers>,
    Path(id): Path<String>,
    Json(ninja_updates): Json<NinjaUpdates>,
) -> Result<Json<Ninja>, (StatusCode, String)> {
    let uuid = match Uuid::parse_str(&id) {
        Ok(u) => u,
        Err(_) => {
...
pub async fn associate_ninja_and_jutsu(
    ...
    let ninja_uuid = match Uuid::parse_str(&ninja_id) {
        ...
        Err(_) => {
            return Err((
                StatusCode::BAD_REQUEST,
                format!("Could not parse uuid: '{}'", ninja_id),
            ))
        }
    };
    let jutsu_uuid = match Uuid::parse_str(&jutsu_id) {
        ...
        Err(_) => {
            return Err((
                StatusCode::BAD_REQUEST,
                format!("Could not parse uuid: '{}'", jutsu_id),
            ))
        }
    };
    match providers
